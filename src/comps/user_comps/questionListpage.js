...
function QuestionListpage(props) {
    ...
    const doApi = async () => {
        ...
        let urlCategory = API_URL + "/category_of_questions/single/" + params.cat_url;
        ...
        let urlProds = API_URL + "/questions/?perPage=8&cat=" + short_id + "&page=" + pageQuery;
        ...
        let urlAmounts = API_URL + "/category_of_questions/amount?cat=" + short_id;
        ...
        <div className='container-fluid' style={{ minHeight: "85vh" }}>
            <div className="container">
                ...
                <div className="row">
                    {ar.map(item => {
                        return (
                            <ProductQuestion key={item._id} item={item} />
                        )
                    })}
                </div>
                {amount < 9 ? "" :
                    <PageLinks perPage="8" apiUrlAmount={API_URL + "/questions/amount?cat=" + shortId} urlLinkTo={"/questions/" + params.cat_url} clsCss="btn btn-dark me-1" />
                }
            </div>
        </div>
    )
